---
name: story-maker-v5-mastra
version: 5.1.0
description: "Standalone-service variant of story-maker-v5: the same Ref2VA pipeline exposed as a Mastra workflow with code-enforced human gates, per-agent model config, and an HTTP API. The Python hands (tools/, scripts/, prompts/, assets/) are verbatim v5; the runbook's orchestration lives in src/mastra/."
triggers:
  - story-maker-v5-mastra
  - story-maker v5 mastra
---

# Story Maker V5 — Mastra service variant

> **This is the standalone-service fork of `story-maker-v5`.** The original
> skill is unchanged at `skills/story-maker-v5/`. Here the SKILL.md runbook
> below is *reference documentation of the pipeline contract* — the actual
> orchestration is a typed Mastra workflow in `src/mastra/` served via HTTP.
> See [`README-MASTRA.md`](README-MASTRA.md) to run it.
